...
def analyze_pipeline_distance_score(logs_path, config_path):
    ...
    # Calculate distances for each pipeline
    distances = []
    for pipeline in task_scores:
        e1, e2, e3 = pipeline
        dist12 = cosine_distances([embeddings_out['extract'][e1]], [embeddings_in['summarize'][e2]])[0][0] if e2 in embeddings_in['summarize'] else np.nan
        dist23 = cosine_distances([embeddings_out['summarize'][e2]], [embeddings_in['classify'][e3]])[0][0] if e3 in embeddings_in['classify'] else np.nan
        total_dist = dist12 + dist23 if not np.isnan(dist12) and not np.isnan(dist23) else np.nan
        if not np.isnan(total_dist):
            distances.append({
                'total_dist': total_dist,
                'task_score': task_scores[pipeline]
            })
    ...
    plt.scatter(distances_df['total_dist'], distances_df['task_score'], alpha=0.7)
    plt.xlabel('Total Distance (Dist12 + Dist23)')
    plt.ylabel('Task Score (Accuracy)')
    plt.title('Task Score vs Total Pipeline Distance')
    plt.grid(True)
    plt.savefig('analysis/pipeline_distance_score_scatter.png')
    plt.show()

    save_cache(cache)

if __name__ == "__main__":
    analyze_pipeline_distance_score("../logs/experiment_logs.csv", "../config.yaml")